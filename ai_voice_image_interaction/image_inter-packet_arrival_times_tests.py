...
data = pd.read_csv("Ossian Image Inter-Packet Arrival Times.csv")
data1 = pd.read_csv("Meta Image Inter-Packet Arrival Times.csv")
    ...
    if packets[i] > 0:
        ...
        if past_time_ossian is not None:
            ...
            if (ossian_time_difference < 400):
                # print(f"Test {o}. time difference is {ossian_time_difference:.2f}")
    ...
    if packets1[i] > 0:
        ...
        if past_time_meta is not None:
            ...
            if (meta_time_difference < 400):
                # print(f"Test {m}. time difference is {meta_time_difference:.2f}")
...
Meta_Glass = plt.plot(sorted_data,  cdf, linewidth=7, color="blue", label="RB-Meta")
Dragon_Glass = plt.plot(sorted_data1,  cdf1, linewidth=7, color="red", label="Dragon")
...
sorted_packets = np.sort(ossian_packets)
sorted_packets1 = np.sort(meta_packets)
...
Meta_Glass = plt.plot(sorted_packets1,  packets_cdf1, linewidth=7, color="blue", label="RB-Meta")
Dragon_Glass = plt.plot(sorted_packets,  packets_cdf, linewidth=7, color="red", label="Dragon")
